...
exec claude ${MESSAGE_ARG} --name "${TITLE}" ${PERMISSIONS_FLAG} ${CAFFEINATE_FLAG}
BOOTSTRAP
else
  cat > "$TMPFILE" << BOOTSTRAP
#!/bin/zsh -l
...
exec claude ${MESSAGE_ARG} --name "${TITLE}" --worktree "${WORKTREE_NAME}" ${PERMISSIONS_FLAG} ${CAFFEINATE_FLAG}
BOOTSTRAP
fi
