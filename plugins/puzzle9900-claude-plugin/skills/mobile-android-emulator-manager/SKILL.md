...
description: Check, create, and launch Android emulators for local automation testing — never uses real devices.
type: mobile
platform: android
---
...
This skill manages Android emulators for local automation testing. It checks if a suitable emulator AVD exists, creates one if needed, and launches it. It is designed to be invoked before running Maestro automation tests to ensure an emulator is available and running.
...
```bash
adb -s emulator-5554 shell pm list packages | grep com.example.waonder
```
